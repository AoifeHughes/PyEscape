...
def make_pts_cuboid_side(endsX, endsY, dist_p):
    ...
    xy = np.around(
        np.array(
            np.meshgrid(
                np.arange(-endsX, endsX, dist_p), np.arange(-endsY, endsY, dist_p)
            )
        )
        .reshape(2, -1)
        .T,
...
def points_on_cuboid(ABC, vol=1, npts=1, faces_dist=np.array([1.0, 1.0, 1.0])):
...
def make_custer_pts_side(ppa, endsX, endsY, pts_per_cluster, ri=None):
    ...
    for pt in pts:
        spirals.append(
            make_distrib_points_2d(
                pts_per_cluster,
                offsetx=pt[0],
                offsety=pt[1],
                ri=(dist_p / 2 if ri is None else ri),
            )
        )
...
def make_clusters_on_cuboid(ABC, vol=1, npts=6, nclusters=6, ri=None):
    if npts < 6 or nclusters < 6:
        raise ValueError(
            "Cuboids have 6 sides, need at least 1 pore per side and nclusters >= npts (npts >= 6, nclusters >= 6)"
        )
    ABC, sas, ta = scale_cuboid(ABC, vol)
    pts = []
    for i, xyz in enumerate([(0, 1), (0, 2), (1, 2)]):
        pts_per_surface = nclusters / 6  # (sas[i]/2 / ta) * npts
        sp_xy = make_custer_pts_side(
            pts_per_surface, ABC[xyz[0]], ABC[xyz[1]], npts / nclusters, ri=ri
        )
...
def random_points_on_cube_surface(samples, r=1):
    ...
    r = np.cbrt(r)
    for i in range(samples):
        p = np.random.random(3) * (r / 2) * np.random.choice([-1, 1], 3)
