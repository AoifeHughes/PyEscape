...
def escape(
    ...
    if inner_r is not None:
        cur_pos[0] = inner_r
        return escape_with_path_inner_sphere(
            r,
            delta,
            dt,
            max_steps,
    ...
    while idx < len(pot_steps):
        ...
        while (not check_func(new_pos, r)) and idx < max_steps:
            if passthrough_pore(new_pos, pore_locs, r=pore_size, tol=tol):
                return (
                    (pot_steps[steps[steps > 0]], (idx + 1) * dt)
                    if track
                    else (idx + 1) * dt
                )
            idx += 1
...
def travel(delta, pa):
    ...
    xyz_sum = np.sum(xyz)
    xyz = np.sqrt(xyz / xyz_sum) * delta * np.random.choice([-1, +1], p.shape)
    p += xyz
    return p


def escape_with_path_inner_sphere(
    r, delta, dt, max_steps, pore_locs, pore_size, check_func, cur_pos, tol, inner_r
):
    ...
    while steps < max_steps:
        ...
        new_pos_steps = 0
        while (
            not (check_func(new_pos, r))
            and new_pos_steps < MAX_NEW_MOVEMENTS
            or np.linalg.norm(new_pos) < inner_r
        ):
            new_pos = travel(delta, cur_pos)
            new_pos_steps += 1
