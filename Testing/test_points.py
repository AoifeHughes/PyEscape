# -*- coding: utf-8 -*-
from PyEscape.escape_points import fibonacci_spheres, random_points_on_cube_surface
...
def test_fibo_spheres_outside():
    assert in_sphere(fibonacci_spheres(P, V)[0], sphere_vol_to_r(V - eps)) is False


def test_ponts_on_cube_inside():
    assert in_cube(random_points_on_cube_surface(P, V)[0], V + eps)


def test_ponts_on_cube_outside():
    assert in_cube(random_points_on_cube_surface(P, V)[0], V - eps) is False
